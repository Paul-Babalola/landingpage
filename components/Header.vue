<template>
  <div class="relative h-screen w-screen">
    ...
    <!-- Navbar -->
    <nav
      class="absolute top-0 left-0 w-full text-white flex items-center justify-between px-5 md:px-64 py-4 z-20"
    >
    ...
    <!-- Overlay content -->
    <div
      class="relative z-10 flex flex-col h-full items-center justify-center text-center px-4 text-white"
    >
      <h1 class="text-2xl md:text-6xl font-bold mb-4 w-3/4 md:w-2/4">
        Creative Home Simpify Your Furniture
      </h1>
      <p
        class="text-sm leading-relaxed max-w-4/5 md:max-w-xl md:text-lg md:leading-loose mt-4 mb-8"
      >
        Do I have consent to record this meeting gain locaion, root-and-branch,
        review, nor game plan who’s the goto
      </p>
      <button
        class="bg-[#E2E2E266] backdrop-blur-md opacity-80 text-white px-15 py-3 rounded-lg font-semibold text-xl"
      >
        Shop Now
      </button>
      <div
        class="absolute bottom-0 left-0 w-full h-20 bg-gradient-to-t from-white to-transparent"
      ></div>
    </div>
  </div>
</template>
