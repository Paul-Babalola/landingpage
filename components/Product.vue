<template>
  <section class="max-w-7xl mx-auto px-4 py-16">
    <h2 class="text-2xl font-bold text-center mb-2">All Product</h2>
    <p class="text-center text-gray-500 max-w-xl mx-auto mb-10">
      The products we provide only for you as our service are selected from the
      best products with number 1 quality in the world
    </p>

    <!-- Product Grid -->
    <div
      class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6"
    >
      <!-- Repeat this block for 8 products -->
      <div class="flex flex-col text-left transition">
        <!-- Bordered box with image + button -->
        <div class="rounded-xl p-4 w-full shadow flex flex-col">
          <img
            src="/assets/ceilinglight.jpg"
            alt="Product"
            class="w-[10rem] h-[14rem] object-contain mb-4 mx-auto"
          />
          <!-- Right aligned button -->
          <div class="flex justify-end">
            <button
              class="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center hover:bg-gray-100"
            >
              <svg
                class="w-4 h-4 text-gray-800"
                fill="none"
                stroke="currentColor"
                stroke-width="2"
                viewBox="0 0 24 24"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  d="M12 4v16m8-8H4"
                />
              </svg>
            </button>
          </div>
        </div>

        <!-- Name & Price outside border, left-aligned -->
        <h3 class="font-semibold text-base md:text-2xl mt-4 mb-1 truncate">
          Ceiling Light
        </h3>
        <div class="text-sm">
          <span class="text-xs font-normal mr-2">$75.00</span>
          <span class="line-through text-xs font-normal text-gray-400"
            >$82.00</span
          >
        </div>
      </div>

      <div class="flex flex-col text-left transition">
        <!-- Bordered box with image + button -->
        <div class="shadow rounded-xl p-4 w-full flex flex-col">
          <img
            src="/assets/chair.png"
            alt="Product"
            class="w-[10rem] h-[14rem] object-contain mb-4 mx-auto"
          />
          <!-- Right aligned button -->
          <div class="flex justify-end">
            <button
              class="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center hover:bg-gray-100"
            >
              <svg
                class="w-4 h-4 text-gray-800"
                fill="none"
                stroke="currentColor"
                stroke-width="2"
                viewBox="0 0 24 24"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  d="M12 4v16m8-8H4"
                />
              </svg>
            </button>
          </div>
        </div>

        <!-- Name & Price outside border, left-aligned -->
        <h3 class="font-semibold text-base md:text-2xl mt-4 mb-1 truncate">
          Wood Chair
        </h3>
        <div class="text-sm">
          <span class="text-xs font-normal mr-2">$50.00</span>
          <span class="line-through text-xs font-normal text-gray-400"
            >$70.00</span
          >
        </div>
      </div>

      <div class="flex flex-col text-left transition">
        <!-- Bordered box with image + button -->
        <div class="shadow rounded-xl p-4 w-full flex flex-col">
          <img
            src="/assets/cupboard.jpg"
            alt="Product"
            class="w-[10rem] h-[14rem] object-contain mb-4 mx-auto"
          />
          <!-- Right aligned button -->
          <div class="flex justify-end">
            <button
              class="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center hover:bg-gray-100"
            >
              <svg
                class="w-4 h-4 text-gray-800"
                fill="none"
                stroke="currentColor"
                stroke-width="2"
                viewBox="0 0 24 24"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  d="M12 4v16m8-8H4"
                />
              </svg>
            </button>
          </div>
        </div>

        <!-- Name & Price outside border, left-aligned -->
        <h3 class="font-semibold text-base md:text-2xl mt-4 mb-1 truncate">
          Paper Cupboard
        </h3>
        <div class="text-sm">
          <span class="text-xs font-normal mr-2">$105.00</span>
          <span class="line-through text-xs font-normal text-gray-400"
            >$120.00</span
          >
        </div>
      </div>

      <div class="flex flex-col text-left transition">
        <!-- Bordered box with image + button -->
        <div class="shadow rounded-xl p-4 w-full flex flex-col">
          <img
            src="/assets/ole.jpg"
            alt="Product"
            class="w-[10rem] h-[14rem] object-contain mb-4 mx-auto"
          />
          <!-- Right aligned button -->
          <div class="flex justify-end">
            <button
              class="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center hover:bg-gray-100"
            >
              <svg
                class="w-4 h-4 text-gray-800"
                fill="none"
                stroke="currentColor"
                stroke-width="2"
                viewBox="0 0 24 24"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  d="M12 4v16m8-8H4"
                />
              </svg>
            </button>
          </div>
        </div>

        <!-- Name & Price outside border, left-aligned -->
        <h3 class="font-semibold text-base md:text-2xl mt-4 mb-1 truncate">
          Ceiling Light
        </h3>
        <div class="text-sm">
          <span class="text-xs font-normal mr-2">$82.00</span>
          <span class="line-through text-xs font-normal text-gray-400"
            >$100.00</span
          >
        </div>
      </div>

      <div class="flex flex-col text-left transition">
        <!-- Bordered box with image + button -->
        <div class="shadow rounded-xl p-4 w-full flex flex-col">
          <img
            src="/assets/Treos.jpg"
            alt="Product"
            class="w-[10rem] h-[14rem] object-contain mb-4 mx-auto"
          />
          <!-- Right aligned button -->
          <div class="flex justify-end">
            <button
              class="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center hover:bg-gray-100"
            >
              <svg
                class="w-4 h-4 text-gray-800"
                fill="none"
                stroke="currentColor"
                stroke-width="2"
                viewBox="0 0 24 24"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  d="M12 4v16m8-8H4"
                />
              </svg>
            </button>
          </div>
        </div>

        <!-- Name & Price outside border, left-aligned -->
        <h3 class="font-semibold text-base md:text-2xl mt-4 mb-1 truncate">
          Treos Sereos 911
        </h3>
        <div class="text-sm">
          <span class="text-xs font-normal mr-2">$200.00</span>
          <span class="line-through text-xs font-normal text-gray-400"
            >$210.00</span
          >
        </div>
      </div>

      <div class="flex flex-col text-left transition">
        <!-- Bordered box with image + button -->
        <div class="shadow rounded-xl p-4 w-full flex flex-col">
          <img
            src="/assets/Multi.jpg"
            alt="Product"
            class="w-[10rem] h-[14rem] object-contain mb-4 mx-auto"
          />
          <!-- Right aligned button -->
          <div class="flex justify-end">
            <button
              class="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center hover:bg-gray-100"
            >
              <svg
                class="w-4 h-4 text-gray-800"
                fill="none"
                stroke="currentColor"
                stroke-width="2"
                viewBox="0 0 24 24"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  d="M12 4v16m8-8H4"
                />
              </svg>
            </button>
          </div>
        </div>

        <!-- Name & Price outside border, left-aligned -->
        <h3 class="font-semibold text-base md:text-2xl mt-4 mb-1 truncate">
          Multi bilderman slibber...
        </h3>
        <div class="text-sm">
          <span class="text-xs font-normal mr-2">$45.00</span>
          <span class="line-through text-xs font-normal text-gray-400"
            >$50.00</span
          >
        </div>
      </div>

      <div class="flex flex-col text-left transition">
        <!-- Bordered box with image + button -->
        <div class="shadow rounded-xl p-4 w-full flex flex-col">
          <img
            src="/assets/xora.jpg"
            alt="Product"
            class="w-[10rem] h-[14rem] object-contain mb-4 mx-auto"
          />
          <!-- Right aligned button -->
          <div class="flex justify-end">
            <button
              class="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center hover:bg-gray-100"
            >
              <svg
                class="w-4 h-4 text-gray-800"
                fill="none"
                stroke="currentColor"
                stroke-width="2"
                viewBox="0 0 24 24"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  d="M12 4v16m8-8H4"
                />
              </svg>
            </button>
          </div>
        </div>

        <!-- Name & Price outside border, left-aligned -->
        <h3 class="font-semibold text-base md:text-2xl mt-4 mb-1 truncate">
          XORA Corner Desk
        </h3>
        <div class="text-sm">
          <span class="text-xs font-normal mr-2">$320.00</span>
          <span class="line-through text-xs font-normal text-gray-400"
            >$325.00</span
          >
        </div>
      </div>

      <div class="flex flex-col text-left transition">
        <!-- Bordered box with image + button -->
        <div class="shadow rounded-xl p-4 w-full flex flex-col">
          <img
            src="/assets/blackforest.jpg"
            alt="Product"
            class="w-[10rem] h-[14rem] object-contain mb-4 mx-auto"
          />
          <!-- Right aligned button -->
          <div class="flex justify-end">
            <button
              class="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center hover:bg-gray-100"
            >
              <svg
                class="w-4 h-4 text-gray-800"
                fill="none"
                stroke="currentColor"
                stroke-width="2"
                viewBox="0 0 24 24"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  d="M12 4v16m8-8H4"
                />
              </svg>
            </button>
          </div>
        </div>

        <!-- Name & Price outside border, left-aligned -->
        <h3 class="font-semibold text-base md:text-2xl mt-4 mb-1 truncate">
          Black Forest Series wo...
        </h3>
        <div class="text-base">
          <span class="text-xs font-normal mr-2">$225.00</span>
          <span class="line-through text-xs font-normal text-gray-400"
            >$240.00</span
          >
        </div>
      </div>
    </div>

    <!-- Pagination Dots & Arrows -->
    <div class="flex items-center justify-center space-x-4 mt-10">
      <button
        class="w-10 h-10 rounded-full border border-gray-300 flex items-center justify-center hover:bg-gray-100"
      >
        <img
          src="/assets/arrow-rightwhite.svg"
          alt="Left Arrow"
          class="w-4 h-4"
        />
      </button>

      <div class="flex space-x-2">
        <span class="w-2.5 h-2.5 rounded-full bg-gray-800"></span>
        <span class="w-2.5 h-2.5 rounded-full bg-gray-300"></span>
        <span class="w-2.5 h-2.5 rounded-full bg-gray-300"></span>
        <span class="w-2.5 h-2.5 rounded-full bg-gray-300"></span>
        <span class="w-2.5 h-2.5 rounded-full bg-gray-300"></span>
      </div>

      <button
        class="w-10 h-10 rounded-full border border-gray-300 flex items-center justify-center hover:bg-gray-100"
      >
        <img
          src="/assets/arrow-right.svg"
          alt="Left Arrow"
          class="rotate-180 w-4 h-4"
        />
      </button>
    </div>
  </section>
</template>
